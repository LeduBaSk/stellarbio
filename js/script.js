...
function updateResultsLanguage() {
  ...
  if (state.lastCalculation) {
    ...
    if (sunSignElement) {
      sunSignElement.innerHTML = `<span class="zodiac-symbol">${sunSymbol}</span> ${getTranslatedZodiacSign(astroData.sunSign)}`;
    }
    if (moonSignElement) {
      moonSignElement.innerHTML = `<span class="zodiac-symbol">${moonSymbol}</span> ${getTranslatedZodiacSign(astroData.moonSign)}`;
    }
    if (risingSignElement) {
      risingSignElement.innerHTML = `<span class="zodiac-symbol">${risingSymbol}</span> ${getTranslatedZodiacSign(astroData.risingSign)}`;
    }
